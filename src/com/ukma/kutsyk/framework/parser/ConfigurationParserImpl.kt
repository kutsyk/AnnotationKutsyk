...
class ConfigurationParserImpl(ConfigurationFile: String = "", private val __ResultList: ArrayList<Bean> = arrayListOf<Bean>()) : ConfigurationParser {
    ...
    init {
        if (Files.exists(Paths.get(ConfigurationFile))) {
            val jsoupDoc = Jsoup.parse(
                    ...
                    Parser.xmlParser())
            jsoupDoc
                    .select("[${ConfigurationConstants.BEAN}] [${ConfigurationConstants.INTERCEPTOR}]")
                    .forEach { xmlBean ->
                        val tmpBean = Bean()
